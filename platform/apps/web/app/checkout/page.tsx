...
function CheckoutContent() {
  ...
  if (paid) {
    return (
      <PageShell>
        <div className="mx-auto max-w-lg rounded-2xl border border-gold/30 bg-cream-deep p-10 text-center">
          <p className="font-display text-3xl font-semibold text-burgundy">Booking Confirmed</p>
          <p className="mt-3 text-ink/60">Confirmation reference {booking.id.slice(0, 8).toUpperCase()}</p>
        </div>
      </PageShell>
    );
  }
    ...
    <PageShell>
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_360px]">
        <div className="flex flex-col gap-8">
          <div>
            <h1 className="font-display text-3xl font-semibold text-burgundy">Complete Your Booking</h1>
          ...
          {error && <div className="rounded-lg border border-burgundy/30 bg-burgundy/5 px-4 py-3 text-sm text-burgundy">{error}</div>}
          ...
          {passenger && booking.flight && (
            <>
              <Section title="Select Seat">
                {seat ? (
                  <p className="text-ink/80">Seat {seat.seatNumber} selected {Number(seat.priceAdjustment) > 0 && `(+$${seat.priceAdjustment})`}</p>
                ) : (
                  <div className="flex flex-col gap-2">
                    {SEAT_ROWS.map((row) => (
                      <div key={row} className="flex items-center gap-2">
                        <span className="w-5 text-xs text-ink/40">{row}</span>
                        {SEAT_LETTERS.map((letter) => (
                          <button
                            key={letter}
                            type="button"
                            onClick={() => selectSeat(`${row}${letter}`)}
                            className="h-9 w-9 rounded-md border border-ink/20 text-xs text-ink/60 transition hover:border-gold hover:text-burgundy"
                          >
                            {row}
                            {letter}
                          </button>
                    ...
                    <p className="mt-1 text-xs text-ink/40">Rows 1–5 carry a $150 preferred-seating fee.</p>
                  </div>
                )}
              </Section>
              ...
              <Section title="Baggage">
                <div className="flex items-center justify-between">
                  <p className="text-ink/70">Checked bags &middot; $120 each</p>
                  <div className="flex items-center gap-4">
                    <span className="text-ink/80">{checkedBagCount}</span>
                    <button
                      type="button"
                      onClick={addCheckedBag}
                      className="rounded-full border border-gold/40 px-3 py-1 text-sm text-burgundy transition hover:bg-gold/10"
                    >
                      + Add bag
                    </button>
                  </div>
        ...
        <aside className="h-fit rounded-2xl border border-gold/30 bg-cream-deep p-7">
          <p className="mb-5 text-xs uppercase tracking-wide text-ink/50">Order Summary</p>
          ...
          <div className="mb-6 flex justify-between font-display text-2xl font-semibold text-burgundy">
            <span>Total</span>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-gold/20 bg-cream-deep/60 p-6">
