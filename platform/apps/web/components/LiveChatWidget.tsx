...
function ChatWidgetPlaceholder() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {open && (
        <div className="w-72 rounded-xl border border-azure/25 bg-panel p-5 shadow-xl">
          <p className="font-display text-lg text-charcoal">We&apos;re here to help</p>
          <p className="mt-2 text-sm text-charcoal/70">
            Live chat connects here once a Crisp account is configured (
            <code className="text-azure">NEXT_PUBLIC_CRISP_WEBSITE_ID</code>).
          </p>
        </div>
      )}
      <button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-azure text-white shadow-lg ring-1 ring-white/40 transition hover:opacity-90"
      >
